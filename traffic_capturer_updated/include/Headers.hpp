...
// ---------------------------------------------------------------------------
// LinuxSLLHeader — 16-byte "cooked" capture header (DLT_LINUX_SLL = 113).
// Used by Docker veth interfaces, loopback, and any interface opened via
// "any". The ethertype sits at bytes 14-15 (offset 14), not at offset 12
// as in Ethernet. packet_parser uses this when datalink == DLT_LINUX_SLL.
// ---------------------------------------------------------------------------
#pragma pack(push,1)
struct LinuxSLLHeader {
    uint16_t pkt_type;       // 0=host, 1=broadcast, 2=multicast, 3=other, 4=outgoing
    uint16_t arphrd_type;    // ARP hardware type
    uint16_t ll_addr_len;    // link-layer address length
    uint8_t  ll_addr[8];     // link-layer address (padded to 8 bytes)
    uint16_t protocol;       // EtherType / protocol

    uint16_t ntoh_protocol() const { return ntohs(protocol); }
};
#pragma pack(pop)
